...
# CPU GIFT CTR Mode Tests
# This script tests the CPU implementation of GIFT in CTR mode
# Uses shared test files from the root tests directory
# Usage: ./test_ctr.sh [--profile]

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
TESTS_DIR="$SCRIPT_DIR/tests"
CPU_CIPHER_DIR="$SCRIPT_DIR/cpu/gift"
RUNS_DIR="$SCRIPT_DIR/runs"

CIPHER="gift"
MODE="ctr"

# Build the cipher binary
echo "Building CPU $CIPHER $MODE..."
cd "$CPU_CIPHER_DIR"
make "${CIPHER}_${MODE}" >/dev/null 2>&1
...
for plaintext_file in $plaintext_files; do
    ...
    while IFS=: read -r key iv; do
        ...
        encrypt_time=$(measure_time "$CPU_CIPHER_DIR/${CIPHER}_${MODE}" -e "$plaintext_path" "$key" "$iv" "$encrypted_file" --nopad 2>/dev/null) || {
            echo "FAILED"
        ...
        decrypt_time=$(measure_time "$CPU_CIPHER_DIR/${CIPHER}_${MODE}" -d "$encrypted_file" "$key" "$iv" "$decrypted_file" --nopad 2>/dev/null) || {
            echo "FAILED"
