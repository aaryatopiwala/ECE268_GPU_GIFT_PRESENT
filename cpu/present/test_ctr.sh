...
for plaintext_file in $plaintext_files; do
    ...
    while IFS=: read -r key iv; do
        ...
        if "$CPU_CIPHER_DIR/${CIPHER}_${MODE}" -e "$plaintext_path" "$key" "$iv" "$encrypted_file" --nopad 2>/dev/null; then
            echo "OK"
        else
            echo "FAILED"
            continue
        ...
        if "$CPU_CIPHER_DIR/${CIPHER}_${MODE}" -d "$encrypted_file" "$key" "$iv" "$decrypted_file" --nopad 2>/dev/null; then
            echo "OK"
        else
            echo "FAILED"
            continue
