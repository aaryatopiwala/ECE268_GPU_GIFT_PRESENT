...
# GPU PRESENT CBC Mode Tests (commented out for future implementation)
# This script tests the GPU implementation of PRESENT in CBC mode
...
for plaintext_file in $plaintext_files; do
    ...
    while IFS=: read -r key iv; do
        ...
        if "$GPU_RUN_SCRIPT" \
            ...
            --key "$key" \
            --iv "$iv" \
            --output "$decrypted_file" 2>/dev/null; then
            echo "OK"
        else
            echo "FAILED"
            continue
